...
TEST(VoxelSnapshotTest, VoxelShapeTest) {
  ...
  for (std::string snapshot_dir : snapshot_files) {
    ...
    const std::string bev_features_path = snapshot_dir + "/bev_features.npy";
    ...
    std::vector<unsigned long> voxels_encoded_shape = raw_voxels_encoded.shape;
    ...
    // check Tensor shape
    EXPECT_EQ(voxels.size(),
              voxel_shape[0] * MAX_NUM_POINTS_PER_PILLAR * NUM_POINT_VALUES);
    EXPECT_EQ(voxel_coord.size(), voxel_coord_shape[0] * 2);
    EXPECT_EQ(
        voxels_encoded.size(),
        voxels_encoded_shape[0] * MAX_NUM_POINTS_PER_PILLAR * FEATURE_NUM);
    EXPECT_EQ(voxels.size() / MAX_NUM_POINTS_PER_PILLAR / NUM_POINT_VALUES,
              voxel_coord.size() / 2);
    EXPECT_EQ(voxels_encoded.size() / MAX_NUM_POINTS_PER_PILLAR / FEATURE_NUM,
              voxel_coord.size() / 2);
...
TEST(VoxelSnapshotTest, VoxelCoordsValueTest) {
  ...
  for (size_t i = 0; i < num_test_files; i++) {
    ...
    std::sort(voxel_coords_snapshot_x.begin(), voxel_coords_snapshot_x.end());
    std::sort(voxel_coords_snapshot_y.begin(), voxel_coords_snapshot_y.end());
